...
/**
 * Smart container component — Planning Pipeline (Kanban Board).
 *
 * Loads all planning applications grouped by status into columns,
 * displays ApplicationCard per application, skeleton loading state,
 * and error state with retry button.
 *
 * Responsibilities:
 * - Dispatches loadApplications on init to populate the store
 * - Groups applications by status into 10 pipeline columns
 * - Shows skeleton loading state while data is being fetched
 * - Shows error state with retry button on failure
 * - Navigates to application detail on card click
 *
 * Requirements: 14.1, 14.2, 14.3, 14.4, 14.5
 */
@Component({
  selector: 'app-planning-pipeline',
  standalone: true,
    ...
    <div class="p-6 space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex flex-col gap-1">
          <h1 class="text-2xl font-bold text-base-content">Planning Pipeline</h1>
          <p class="text-sm text-base-content/60">
            Track planning applications across lifecycle stages. Click an application to view details.
          </p>
        </div>
        <div class="flex items-center gap-2">
          <span class="badge badge-neutral badge-outline text-xs">
            {{ getTotalCount() }} total applications
          </span>
        </div>
      </div>
      ...
      <ng-container *ngIf="error$ | async as error">
        ...
        >
          ...
          <button
            ...
            (click)="onRetry()"
            aria-label="Retry loading planning pipeline data"
          >
      ...
      <ng-container *ngIf="(loading$ | async) && !(error$ | async)">
        <div
          ...
          aria-label="Loading planning pipeline data"
          role="status"
        >
          <div
            ...
            <div class="flex flex-col gap-3 p-3">
              ...
              >
                <div class="card-body p-4 space-y-2">
                  ...
                  <div class="h-3 w-1/2 bg-base-300 rounded"></div>
          ...
          <span class="sr-only">Loading planning pipeline data, please wait...</span>
        </div>
      </ng-container>
      ...
      <ng-container *ngIf="!(loading$ | async) && !(error$ | async)">
        <div
          class="flex gap-4 overflow-x-auto pb-4 -mx-2 px-2"
          role="region"
          aria-label="Planning application pipeline board"
        >
          @for (column of pipelineColumns; track column.status) {
            <app-planning-pipeline-column
              [status]="formatStatusLabel(column.status)"
              [count]="getColumnApplications(column.status).length"
              [applications]="getColumnApplications(column.status)"
...
export class PlanningPipelineComponent implements OnInit, OnDestroy {
  private readonly store = inject(Store);
  private readonly router = inject(Router);
  private subscription: Subscription | null = null;

  /** Observable of applications grouped by status for the pipeline view. */
  readonly applicationsByStatus$: Observable<Record<PlanningApplicationStatus, readonly IApplicationListItem[]>> =
    this.store.select(selectApplicationsByStatus);

  /** Observable of loading state. */
  readonly loading$: Observable<boolean> = this.store.select(selectApplicationLoading);

  /** Observable of error state. */
  readonly error$: Observable<string | null> = this.store.select(selectApplicationError);

  /** Pipeline columns definition in lifecycle order. */
  readonly pipelineColumns: readonly PipelineColumnDef[] = [
    { status: PlanningApplicationStatus.PreApplication },
    { status: PlanningApplicationStatus.Submitted },
    { status: PlanningApplicationStatus.Validated },
    { status: PlanningApplicationStatus.UnderReview },
    { status: PlanningApplicationStatus.CommitteeReview },
    ...
    { status: PlanningApplicationStatus.Withdrawn }
  ];
  ...
  readonly skeletonColumns = [
    ...
    { cards: [{ titleWidth: '75%' }, { titleWidth: '60%' }, { titleWidth: '80%' }] },
    { cards: [{ titleWidth: '70%' }] },
    { cards: [{ titleWidth: '60%' }, { titleWidth: '85%' }] },
    { cards: [{ titleWidth: '75%' }] },
    { cards: [{ titleWidth: '55%' }, { titleWidth: '70%' }] }
  ];

  /** Cached snapshot of grouped applications for synchronous template access. */
  private groupedApplications: Record<PlanningApplicationStatus, readonly IApplicationListItem[]> = {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.applicationsByStatus$.subscribe(
      (grouped: Record<PlanningApplicationStatus, readonly IApplicationListItem[]>) => {
  ...
  /**
   * Returns the applications array for a given status column.
   */
  getColumnApplications(status: PlanningApplicationStatus): IApplicationListItem[] {
    return [...(this.groupedApplications[status] ?? [])];
  }
  ...
  getTotalCount(): number {
    return Object.values(this.groupedApplications).reduce(
      (total, applications) => total + applications.length,
      0
    );
  }

  /**
   * Navigates to the application detail page when a card is clicked.
   */
  onCardClick(application: IApplicationListItem): void {
    this.router.navigate(['/planning-approvals', 'applications', application.id]);
  }

  /**
   * Retries loading applications after a failure.
   */
  onRetry(): void {
    this.store.dispatch(ApplicationActions.loadApplications());
  }
