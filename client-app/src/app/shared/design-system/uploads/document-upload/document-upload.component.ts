...
/**
 * Document type option for the document type selector.
 */
...
/**
 * Unified DocumentUploadComponent — A configurable document upload component with
 * drag-and-drop support, file type/size validation, document type selection,
 * upload progress indicator, and error display.
 *
 * Consolidates upload components from:
 * - Land Acquisition (full featured: drag-and-drop, progress, validation)
 * - Legal Compliance (simpler form-based upload)
 *
 * The `documentTypes` input is configurable so any module can pass its own type list.
 * The `uploadUrl` input configures the API endpoint.
 *
 * @example
 * ```html
 * <app-document-upload
 *   [entityId]="opportunity.id"
 *   [documentTypes]="landDocTypes"
 *   uploadUrl="/api/v1/documents"
    ...
    <div class="card bg-base-100 shadow-sm border border-base-300">
      <div class="card-body p-5">
        ...
        <p class="text-sm text-base-content/60 mb-4">
          Attach documents to this entity. Allowed formats: PDF, DOCX, XLSX, PNG, JPG (max 25 MB).
        </p>
...
export class DocumentUploadComponent {
  ...
  /** The entity ID to attach the document to (generic — works for any entity). */
  @Input({ required: true }) entityId!: string;

  /** Configurable list of document types for the selector dropdown. */
  @Input({ required: true }) documentTypes: readonly IDocumentTypeOption[] = [];

  /** The API endpoint URL for uploading documents. */
  @Input({ required: true }) uploadUrl = '';

  /** Emitted when a document is successfully uploaded. Payload is the API response data. */
  ...
  /** Trigger the upload via HttpClient. */
  onUpload(): void {
    const file = this.selectedFile();
    const docType = this.selectedDocType();

    if (!file || !docType || this.isUploading()) return;
